...
const _getCode = ({ ssc, mc, hc, code }) => {
  const responseCode = new Number(`${Constants.SERVICE_CODE}${ssc}${mc}${hc}${code}`);
...
const _getError = (err) => {
  ...
  return { code: _getCode(err), message: _getStrError(err), errLogs: _getErrLogs(err) };
};
...
const HTTP_CODE = {
  ...
  _404: { httpCode: 404, type: 'NOT_FOUND', code: 401, message: 'not found', ...UN_SUCCESS },
